...
void camera_task(void* pdata){
	...
	while(1){
		...
		if (cam_debug){
			picture = fopen(CAM_PICTURE_OUTPUT[pic_count], "w");
		}else{
		...
		*/
		...
		if (synced){
			...
			//Send reset and try to sync again
...
//				printf("Packet %i bad, moving on\n", q);
		...
		fclose(picture);
		if(cam_debug){
			picture = fopen(CAM_PICTURE_OUTPUT[pic_count], "r");
			pic_count++;
		}else{
		...
		if(cam_debug){
			OSQPost(findCommandQueue, (void*) colour);
		}else{
